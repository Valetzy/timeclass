            ...
            <script>
                ...
                setInterval(() => {
                    ...
                    document.querySelectorAll('.card').forEach(card => {
                        ...
                        const startTime = card.querySelector('button').getAttribute('data-start_time');
                        const endTime = card.querySelector('button').getAttribute('data-end_time');
                        ...
                        // Calculate 10 minutes before start and end times
                        const startAlarmTime = new Date(startDateTime.getTime() - 10 * 60 * 1000);
                        const endAlarmTime = new Date(endDateTime.getTime() - 10 * 60 * 1000);

                        // Check if current time is equal to the alarm time for start or end
                        if (
                            currentTime.getHours() === startAlarmTime.getHours() &&
                            currentTime.getMinutes() === startAlarmTime.getMinutes()
                        ) {
                            alert(`Reminder: ${subject} starts in 10 minutes!`);
                        }

                        if (
                            currentTime.getHours() === endAlarmTime.getHours() &&
                            currentTime.getMinutes() === endAlarmTime.getMinutes()
                        ) {
                            alert(`Reminder: ${subject} ends in 10 minutes!`);
                        }
                    });
