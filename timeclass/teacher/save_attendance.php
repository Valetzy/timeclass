...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['data'], $_POST['s_id'], $_POST['subject'], $_POST['c_id'], $_POST['t_id'], $_POST['start_time'], $_POST['end_time'], $_POST['room'], $_POST['floor'])) {
            ...
            if ($currentTime >= $start_time && $currentTime <= $end_time) {
                ...
                if ($checkStmt = $conn->prepare($checkSql)) {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $sql = "INSERT INTO attendance (s_id, subject, c_id, t_id, scanned_data, start_time, end_time, room, floor, timestamp, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        if ($stmt = $conn->prepare($sql)) {
                            $stmt->bind_param('ssissssssss', $s_id, $subject, $c_id, $t_id, $scannedData, $start_time, $end_time, $room, $floor, $currentTimestamp, $status);

                            if ($stmt->execute()) {
                                echo "<script>alert('Attendance Successfully.'); window.location='teacher_db.php';</script>";
                            } else {
